...
import polyflow
from polyflow.types import RerankerOutput, RMOutput


@pd.api.extensions.register_dataframe_accessor("vector_search")
class SemSearchDataframe:
    ...
    def __call__(
        ...
        if n_rerank is not None:
            if polyflow.settings.reranker is None:
                raise ValueError("SemanticReranker not found in settings")

            docs = new_df[col_name].tolist()
            reranked_output: RerankerOutput = polyflow.settings.reranker(query, docs, n_rerank)
