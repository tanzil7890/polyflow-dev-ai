...
import polyflow
from polyflow.models import RetrieverEngine
...
class SemSimJoinDataframe:
    ...
    def __call__(
        ...
        if not isinstance(rm, RetrieverEngine):
            raise ValueError(
                "The retrieval model must be an instance of RetrieverEngine. Please configure a valid retrieval model using polyflow.settings.configure()"
            )
