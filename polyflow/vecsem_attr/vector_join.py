...
from polyflow.types import CascadeArgs, SemanticJoinOutput
...
from .vector_filter import vector_filter
...
def vector_join(
    ...
    Args:
        ...
        model (polyflow.models.LM): The model to use.
    ...
    if safe_mode:
        ...
        estimated_tokens_per_call = model.count_tokens(
            polyflow.templates.task_instructions.filter_formatter(
    ...
    polyflow.logger.debug(f"outputs: {filter_outputs}")
    polyflow.logger.debug(f"explanations: {all_explanations}")
...
def sem_join_cascade(
    ...
    if safe_mode:
        ...
        polyflow.logger.warning("Safe mode is not implemented yet.")
...
def run_sem_sim_join(l1: pd.Series, l2: pd.Series, col1_label: str, col2_label: str) -> pd.DataFrame:
    """
    Wrapper function to run sem_sim_join in vector_join then calibrate the scores for approximate join
    ...
    if isinstance(l1, pd.Series):
        ...
        polyflow.logger.error("l1 must be a pandas Series or DataFrame")

    l2_df = l2.to_frame(name=col2_label)
    l2_df = l2_df.vector_index(col2_label, f"{col2_label}_index")
...
def map_l1_to_l2(
    ...
    """
    Wrapper function to run sem_map in vector_join.
...
def join_optimizer(
    ...
    if polyflow.settings.helper_lm is not None:
        polyflow.logger.debug("Helper model is not supported yet. Default to similarity join.")
    ...
    polyflow.logger.info("Join Optimizer: plan cost analysis:")
    polyflow.logger.info(f"    Search-Filter: {sf_cost} LLM calls.")
    polyflow.logger.info(
        f"    Search-Filter: accept {len(sf_high_conf)} helper positive results, {sf_high_conf_neg} helper negative results."
    )
    polyflow.logger.info(f"    Map-Search-Filter: {msf_cost} LLM calls.")
    polyflow.logger.info(
    ...
    if sf_cost < msf_cost:
        polyflow.logger.info("Proceeding with Search-Filter")
        ...
        polyflow.logger.info("Proceeding with Map-Search-Filter")
...
def learn_join_cascade_threshold(
    ...
    Args:
        ...
        col1_label (str): The label for the first column.
        col2_label (str): The label for the second column.
        model (polyflow.models.LM): The language model.
        user_instruction (str): The user instruction for join.
        cascade_args (CascadeArgs): The cascade arguments.
        examples_multimodal_data (list[dict[str, Any]] | None): The examples multimodal data. Defaults to None.
        examples_answers (list[bool] | None): The answers for examples. Defaults to None.
    ...
    polyflow.logger.info(f"Sampled {len(sample_indices)} out of {len(helper_scores)} helper join results.")
    ...
    try:
        ...
        polyflow.logger.info(f"Learned cascade thresholds: {(pos_threshold, neg_threshold)}")

    except Exception as e:
        polyflow.logger.error(f"Error while learning filter cascade thresholds: {e}")
        polyflow.logger.error("Default to full join.")
        return 1.0, 0.0, len(sample_indices)

    return pos_threshold, neg_threshold, len(sample_indices)


@pd.api.extensions.register_dataframe_accessor("vector_join")
class SemJoinDataframe:
    ...
    def __call__(
        ...
        cols = polyflow.nl_expression.parse_cols(join_instruction)
        left_on = None
        right_on = None
        for col in cols:
            if ":left" in col:
        ...
        polyflow.logger.debug(f"join_results: {join_results}")
        polyflow.logger.debug(f"all_raw_outputs: {all_raw_outputs}")
