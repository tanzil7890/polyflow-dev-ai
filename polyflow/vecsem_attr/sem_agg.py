...
from polyflow.types import LMOutput, SemanticAggOutput
    ...
    while len(docs) != 1 or summaries == []:
        ...
        for idx in range(len(docs)):
            ...
            if (new_tokens + context_tokens + template_tokens > model.max_ctx_len - model.max_tokens) or (
                ...
                polyflow.logger.debug(f"Prompt added to batch: {prompt}")
        ...
        docs = summaries
        polyflow.logger.debug(f"Model outputs from tree level {tree_level}: {summaries}")
        tree_level += 1
        if safe_mode:
...
class SemAggDataframe:
    ...
    def __call__(
        ...
        polyflow.logger.debug(f"User instruction: {user_instruction}")
        if all_cols:
        ...
        polyflow.logger.debug(f"df_txt: {df_txt}")
        formatted_usr_instr = polyflow.nl_expression.nle2str(user_instruction, col_li)
        polyflow.logger.debug(f"formatted_usr_instr: {formatted_usr_instr}")
