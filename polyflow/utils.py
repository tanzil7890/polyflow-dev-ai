...
def compute_cosine_similarity(vectors: NDArray[np.float64], query_vector: NDArray[np.float64]) -> NDArray[np.float64]:
    """Compute cosine similarity between vectors and query vector."""
    norm = np.linalg.norm(vectors, axis=1) * np.linalg.norm(query_vector)
    return np.dot(vectors, query_vector.T).flatten() / norm
