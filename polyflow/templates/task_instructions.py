...
import pandas as pd
import polyflow
from polyflow.dtype_extensions import ImageDtype
...
def df2text(df: pd.DataFrame, cols: list[str]) -> list[str]:
    ...
    if polyflow.settings.serialization_format == SerializationFormat.DEFAULT:
        formatted_rows = projected_df.apply(lambda x: custom_format_row(x, cols), axis=1).tolist()
    elif polyflow.settings.serialization_format == SerializationFormat.JSON:
        formatted_rows = projected_df.to_json(orient="records", lines=True).splitlines()
    elif polyflow.settings.serialization_format == SerializationFormat.XML:
        try:
